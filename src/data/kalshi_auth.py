"""Kalshi RSA-PSS request signing. Lifted unchanged from the prior bot — it works."""
...
def _sign_pss_base64(private_key_raw: str, message: str) -> str:
    ...
    signature = private_key.sign(message.encode("utf-8"), _pss_padding(), _sha256_hash())
    ...
    if len(key_str) < 240:
        ...
        try:
            if path.exists() and path.is_file():
                return _deserialize(path.read_bytes(), serialization, default_backend)
        except OSError:
            pass
    if "BEGIN" in key_str:
        return _deserialize(key_str.encode("utf-8"), serialization, default_backend)
    try:
        compact = "".join(key_str.split())
        padding_eq = "=" * ((4 - len(compact) % 4) % 4)
        der_bytes = base64.b64decode(compact + padding_eq, validate=False)
...
def _deserialize(key_bytes, serialization, default_backend):
    try:
...
def _pss_padding():
    ...
    return padding.PSS(mgf=padding.MGF1(_sha256_hash()), salt_length=padding.PSS.DIGEST_LENGTH)
